...
The automated suite covers benchmark lifecycle, CSV metadata and columns, captured settings, filename collision, write-failure and shutdown-footer handling, complete summary calculations and formatting, all three pause modes, configuration fallback and validation, and deterministic metric calculations for average FPS, frametime, percentile lows, mean-worst lows, stutter threshold inclusion and percentage, maximum spike, reset behavior, and rolling-window boundary inclusion.
