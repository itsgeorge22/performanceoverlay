...
- Run `versions/26.2/gradlew.bat -p versions/26.2 runClientGameTest` on Windows or `bash ./versions/26.2/gradlew -p versions/26.2 runClientGameTest` elsewhere after changes specific to the Java 25 Minecraft 26.2 build line.
- Run `gradlew.bat testSupportedVersions` on Windows or `./gradlew testSupportedVersions` elsewhere after compatibility-sensitive changes and before every release. This is the authoritative automated check for every Minecraft version declared by all current JARs.
